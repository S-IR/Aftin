...
export default async function handler (req: NextApiRequest, res: NextApiResponse){
  ...
  if(!category || !subCat) return res.status(406).send(`not acceptable criteria`)
  ...
  return res.setHeader('Cache-Control', 'public, max-age=7200, s-maxage=7200').status(200).send(docsArray)
}