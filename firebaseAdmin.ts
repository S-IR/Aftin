 ...
 * @returns 
 */
export async function sendVerificationEmail(userEmail: string, actionCodeSettings: ActionCodeSettings) {
try {
...
export async function sendPasswordReset(userEmail: string, actionCodeSettings: ActionCodeSettings) {
  try {
    const isValidEmail = admin.auth().fetch
    const actionLink = await admin.auth().generatePasswordResetLink(userEmail, actionCodeSettings)
    const template = await ejs.renderFile('./emails/verify-email.ejs', {
      actionLink,
      randomNumber: Math.random()
    })
