...
export const ADD_SHAPE: CaseReducer<WritableDraft<canvasState>, PayloadAction<addShape>> = (state, action) => {
...
export const ADD_SHAPE_PATTERN_IMAGE: CaseReducer<WritableDraft<canvasState>, PayloadAction<addShapePatternImage>> = (state, action) => {
...
export const REMOVE_SHAPE_PATTERN_IMAGE: CaseReducer<WritableDraft<canvasState>, PayloadAction<removeShapePatternImage>> = (state, action) => {
...
export const ADD_SHAPE_FILTER: CaseReducer<WritableDraft<canvasState>, PayloadAction<addShapeFilter>> = (state, action) => {
  const { pageId, data } = action.payload
  state[pageId].push(data)
}


export const CHANGE_SHAPE_FILL: CaseReducer<WritableDraft<canvasState>, PayloadAction<changeShapeFill>> = (state, action) => {
  const { pageId, elementId, fill } = action.payload
  console.log(`CHANGE_SHAPE_FILL: pageId, elementId, fill`, pageId, elementId, fill);
  
  state[pageId][elementId].filter.fill = fill
}

export const CHANGE_TEXT_STROKE_COLOR: CaseReducer<WritableDraft<canvasState>, PayloadAction<changeTextStrokeColor>> = (state, action) => {
  const { pageId, elementId, stroke } = action.payload
  const selectedElement: shapeFilter = state[pageId][elementId]
