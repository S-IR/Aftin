...
import { filterState, textFilter } from "./filtersHandlingReducers";
...
export const ADD_TEXT: CaseReducer<
  WritableDraft<canvasState>,
  PayloadAction<addText>
> = (state, action) => {
...
export const CHANGE_TEXT: CaseReducer<
  WritableDraft<canvasState>,
  PayloadAction<changeText>
> = (state, action) => {
...
export const CHANGE_FONT_FAMILY: CaseReducer<
  WritableDraft<canvasState>,
  PayloadAction<changeFontFamily>
> = (state, action) => {
  const { pageId, elementId, fontFamily } = action.payload;
  const selectedElement: textData = state.pages[pageId][elementId].data;
  selectedElement.fontFamily = fontFamily;
};
export const CHANGE_FONT_SIZE: CaseReducer<
  WritableDraft<canvasState>,
  PayloadAction<changeFontSize>
> = (state, action) => {
...
export const CHANGE_FONT_VARIANT: CaseReducer<
  WritableDraft<canvasState>,
  PayloadAction<changeFontVariant>
> = (state, action) => {
...
export const ADD_TEXT_FILTER: CaseReducer<
  WritableDraft<filterState>,
  PayloadAction<addTextFilter>
> = (state, action) => {
  const { pageId, data } = action.payload;
  state.filters[pageId].push(data);
};

export const CHANGE_FONT_COLOR: CaseReducer<
  WritableDraft<filterState>,
  PayloadAction<changeFontColor>
> = (state, action) => {
  const { pageId, elementId, fill } = action.payload;
  const selectedElement: textFilter = state.filters[pageId][elementId];
