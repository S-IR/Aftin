...
interface props {
  selectedColorPallet: string | undefined
  setSorts: React.Dispatch<React.SetStateAction<SidebarSorts>>
}

const SortColorPallet = ({ selectedColorPallet, setSorts }: props) => {

  const [open, setOpen] = useState(false)
  const handleClick = () => setOpen((open) => (!open))
  const handleOptionClick = (value: string | object) => setSorts((sorts: SidebarSorts) => {
    if(typeof(sorts.colorPallet) === `object` && typeof(value) !== `object`) delete sorts.colorPallet
    return { ...sorts, colorPallet: value}
  })
  ...
  return (
    <>
      ...
      >
        <ListItemIcon>
          <ColorLens style={{ color: 'green', width: 32, height: 32 }}
          />
        </ListItemIcon>
...
export default SortColorPallet