...
import { type_sweets_and_desserts } from '../../typings/image-types/SweetsAndDesertsTypes'
...
type Inputs = {
  ...
  type: type_sweets_and_desserts
  resolution : string
}

const UploadSweetsAndDesserts = () => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const doc = collection(db, `Stock Images/Desserts & Sweets/Images`)
  const storage = getStorage()
  // the upload name is a random string
  
  const storageRef = ref(storage, `public-images/stock-images/desserts-and-sweets/${makeID(16)}.png`)
  const views = 0
...
export default UploadSweetsAndDesserts