import { appetizers_array, artwork_styles_array, banner_type_array, drinks_array, fast_foods_array, fruits_array, LARGE_CATEGORY_OF_IMG, logo_type_array, main_dish_array, material_type, menu_size_array, shape_array, size_array, SMALL_CATEGORY_OF_IMG, soups_array, spices_array, stickers_and_cliparts_categories, surr_env_array, sweets_and_desserts_array, utensils_and_plates_array, vegetables_array, } from "../../typings/image-types/ImageTypes";
...
export const determineInputs = (
  ...
  const Array = []
  Array.push({color_scheme: ``}, {description: ``})
  ...
  switch (SMALL_CATEGORY_OF_IMG) {
    case `appetizers`: 
    Array.push({dish: appetizers_array})
    break;
    case `soups`:
    ...
    case `utensils-and-plates`:
      Array.push({ type: utensils_and_plates_array })
    ...
    case `ingredients`:
      Array.push({
        ingredients:
          [
            { spices: { spice_type: spices_array } },
            { fruits: { fruit_type: fruits_array } },
            { vegetables: { vegetable_type: vegetables_array } },
            `flour`, `syrup`
          ]
      })
      break;
    ...
    case `stickers-and-cliparts`:
      Array.push({ category: stickers_and_cliparts_categories })
      break;
    case `brochures`:
