...
import { GrahicDesignsOptions, GraphicDesignType, ImgDoc, SMALL_CATEGORY_OF_IMG, StockImagesOptions, StockImageType, subCats_array, valid_image_fields } from "../../typings/image-types/ImageTypes"
 ...
 * @param {'graphic designs | stock-images'} category the category of images
...
export const requestImageDocs = async (rowRequested: number = 0, category: `graphic-designs` | `stock-images`, subCat: string ,queryData: ParsedUrlQuery) => {
  ...
  if(isValidQuery) {
    const res : Response = await fetch(`${process.env.NEXT_PUBLIC_server}/api/requestImagesAPI?${new URLSearchParams({
      category,
      subCat,
